...
public class ParkingLot {
	...
	private HashMap<Vehicle.VehicleSize, HashMap<Vehicle.VehicleType, ArrayList<ParkingSpace>>> parkingAvailableBySize = new HashMap<>();
	...
	public ParkingLot(int numberOfSmallRegularSpaces, int numberOfMediumRegularSpaces, int numberOfLargeRegularSpaces,
			int numberOfSmallHandicappedSpaces, int numberOfMediumHandicappedSpaces,
		...
		for (int sizeIndex = 0; sizeIndex < vehicleSizes.length; sizeIndex++) {
			for (int vehicleTypeIndex = 0; vehicleTypeIndex < vehicleTypes.length; vehicleTypeIndex++) {
				...
				if (parkingAvailableBySize.containsKey(vehicleSizes[sizeIndex])) {
					HashMap<Vehicle.VehicleType, ArrayList<ParkingSpace>> previousTypeToSpaceMap = parkingAvailableBySize
							.get(vehicleSizes[sizeIndex]);
	...
	public ParkingLot(
			HashMap<Vehicle.VehicleSize, HashMap<Vehicle.VehicleType, ArrayList<ParkingSpace>>> parkingAvaliableBySize) {
	...
	private ParkingSpace tryToPark(Vehicle vehicle) {
		...
		if (size.equals(Vehicle.VehicleSize.LARGE)) {
			...
			if (smallSpace != null) {
				...
				// Check medium spaces
				ParkingSpace mediumSpace = findAvaliableSpaceWithType(type,
						parkingAvailableBySize.get(Vehicle.VehicleSize.MEDIUM));
				if (mediumSpace != null) {
					return mediumSpace;
				} else {
					// Check large spaces --> if this is null, then no appropriate spaces are
					// avaliable
					return findAvaliableSpaceWithType(type, parkingAvailableBySize.get(Vehicle.VehicleSize.LARGE));
				}
			}
	...
	private ParkingSpace findAvaliableSpaceWithType(Vehicle.VehicleType vehicleType,
			HashMap<Vehicle.VehicleType, ArrayList<ParkingSpace>> parkingAvailableByType) {
		if (vehicleType.equals(Vehicle.VehicleType.HANDICAP)) {
	...
	private int toStringHelper_AvaliableSpacesWithTypeAndSize(Vehicle.VehicleType vehicleType,
		...
		HashMap<Vehicle.VehicleType, ArrayList<ParkingSpace>> avaliableByTypeAndSize = parkingAvailableBySize
				.get(vehicleSize);
